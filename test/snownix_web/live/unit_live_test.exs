defmodule SnownixWeb.UnitLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all units", %{conn: conn, unit: unit} do
      {:ok, _index_live, html} = live(conn, Routes.admin_unit_index_path(conn, :index))
    ...
    test "saves new unit", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.admin_unit_index_path(conn, :index))

      assert index_live |> element("a", "New Unit") |> render_click() =~
               "New Unit"

      assert_patch(index_live, Routes.admin_unit_index_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.admin_unit_index_path(conn, :index))
    ...
    test "updates unit in listing", %{conn: conn, unit: unit} do
      ...
      assert_patch(index_live, Routes.admin_unit_index_path(conn, :edit, unit))
  ...
  describe "Show" do
    ...
    test "displays unit", %{conn: conn, unit: unit} do
      {:ok, _show_live, html} = live(conn, Routes.admin_unit_show_path(conn, :show, unit))
    ...
    test "updates unit within modal", %{conn: conn, unit: unit} do
      {:ok, show_live, _html} = live(conn, Routes.admin_unit_show_path(conn, :show, unit))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Unit"

      assert_patch(show_live, Routes.admin_unit_show_path(conn, :edit, unit))
      ...
      {:ok, _, html} =
        ...
        |> form("#unit-form", unit: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.admin_unit_show_path(conn, :show, unit))
