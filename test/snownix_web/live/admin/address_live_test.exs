defmodule SnownixWeb.Admin.AddressLiveTest do
  ...
  @create_attrs %{city: "some city", country: "some country", currency: "some currency", fax: "some fax", phone: "some phone", state: "some state", street: "some street", street_2: "some street_2", zip: "some zip"}
  ...
  describe "Index" do
    ...
    test "lists all invoice_addresses", %{conn: conn, address: address} do
      {:ok, _index_live, html} = live(conn, Routes.admin_address_index_path(conn, :index))

      assert html =~ "Listing Invoice addresses"
