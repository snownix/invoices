defmodule Snownix.AccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      ...
      assert %{
               ...
               lastname: ["lastname must be alphanumeric"],
    ...
    test "validates maximum values for email and password for security" do
      ...
      assert "should be at most 40 character(s)" in errors_on(changeset).firstname
      assert "should be at most 40 character(s)" in errors_on(changeset).lastname
    ...
    test "registers users with a hashed password" do
      ...
      {:ok, user} =
        Accounts.register_user(
          valid_user_attributes(email: email, firstname: @firstname, lastname: @lastname)
        )
  ...
  describe "user_register_changeset/2" do
    test "returns a changeset" do
      ...
      assert changeset.required == [:firstname, :lastname, :email, :password]
    end
    ...
    test "allows fields to be set" do
      ...
      assert get_change(changeset, :firstname) == @firstname
      assert get_change(changeset, :lastname) == @lastname
  ...
  describe "update_user/3" do
    ...
    test "validates current password", %{user: user} do
      {:error, changeset} = Accounts.update_user(user, "invalid", %{firstname: "Jone"})

      assert %{current_password: ["Wrong password"]} = errors_on(changeset)
    end

    test "updates with a valid password", %{user: user} do
      firstname = "update name with valid password"
      ...
      assert is_nil(user.password)
      assert Accounts.get_user_by_email(user.email).firstname == firstname
    end

    test "update only firstname, lastname and phone information", %{user: user} do
      Accounts.update_user(user, valid_user_password(), %{
        phone: "phone",
        firstname: "firstname",
        lastname: "lastname"
      ...
      fresh_user = Accounts.get_user_by_email!(user.email)
      assert fresh_user.phone == "phone"
      assert fresh_user.firstname == "firstname"
      assert fresh_user.lastname == "lastname"
    end
  end
